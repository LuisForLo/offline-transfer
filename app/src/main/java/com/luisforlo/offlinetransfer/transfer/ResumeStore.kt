 ...
 *
 * The partial payload lives under filesDir/resumable-transfers and survives
 * Activity/process restarts and app updates. A small sidecar manifest lets the
 * app recover the exact identity even after a completely new E2E session.
 */
object ResumeStore {
    ...
    fun list(context: Context): List<Entry> {
        ...
        return dir.listFiles { file -> file.name.endsWith(META_SUFFIX) }
            .orEmpty()
            .mapNotNull { meta -> readEntry(meta) }
