...
class TcpFileTransferTest {
    ...
    fun transfersFileAndReturnsReceiverHashAcknowledgement() {
        ...
        try {
            ...
            assertTrue(senderResult.transferElapsedMillis >= 1L)
            assertTrue(receiverResult.transferElapsedMillis >= 1L)
            assertTrue(senderResult.bytesPerSecond > 0.0)
            assertTrue(receiverResult.bytesPerSecond > 0.0)
            assertTrue(senderResult.effectiveSendBufferBytes > 0)
            assertTrue(senderResult.effectiveReceiveBufferBytes > 0)
            assertTrue(receiverResult.effectiveSendBufferBytes > 0)
            assertTrue(receiverResult.effectiveReceiveBufferBytes > 0)
